...
-- 3) 가장 오래 근속한 직원의 입사일은 언제인가요? 다음 형식으로 출력해주세요. 예) 2014년 07월 10일
...
select min(date_format(curdate(), '%Y') - date_format(birth_date, '%Y')+1) as '최고 어린 사원',
       max(date_format(curdate(), '%Y') - date_format(birth_date, '%Y')+1) as '최 연장자'
from employees a, titles b
where a.emp_no = b.emp_no
  and b.to_date = '9999-01-01';
